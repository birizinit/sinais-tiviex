...
{LINK_CORRETORA}
"""
    await application.bot.send_message(chat_id=CHAT_ID, text=mensagem, parse_mode="Markdown")
    ...
    if is_win:
        placar["WIN"] += 1
        await application.bot.send_sticker(chat_id=CHAT_ID, sticker=STICKER_WIN)
...
➡️ Clique aqui para acessar a corretora 👇🏻
{LINK_CORRETORA}"""
        await application.bot.send_message(chat_id=CHAT_ID, text=mensagem_loss)
...
➡️ Clique aqui para acessar a corretora 👇🏻
{LINK_CORRETORA}"""
    await application.bot.send_message(chat_id=CHAT_ID, text=texto, parse_mode="Markdown")
...
async def executar_sessao(nome_sessao, hora, minuto, live=False):
    ...
    if not live:
        ...
        await application.bot.send_sticker(chat_id=CHAT_ID, sticker=STICKER_INICIO)
    ...
    if not live:
        ...
        await application.bot.send_sticker(chat_id=CHAT_ID, sticker=STICKER_FIM)
